#!/bin/bash
# set file path
NGINX_ACCESS_LOG=/mydata/logs/nginx/access/log.pipe
...
echo 'start stat'
O_LOG_FILE=$NGINX_ACCESS_LOG.`date -d yesterday +%Y%m%d`
yestoday=`date -d"1 day ago today" +%Y-%m-%d`
grep -v 123.206.1.186 $O_LOG_FILE|grep guid >> $LOG_DIR/$yestoday.log
