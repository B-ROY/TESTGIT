...
class UcpaasSMS:
	...
	def __templateSMS(self,toNumbers,templateId,param,isUseJson=True):
		...
		if isUseJson == True:
			body = '{"templateSMS":{ "appId":"%s","to":"%s","templateId":"%s","param":"%s"}}'%(self.__appId,toNumbers,templateId,param)
	...
	def get_access_token(self,phoneNumber,openId,create_time=None):
		...
		body = '{"Accid":"%s","AccToken":"%s","Cnumber":"%s","Cpwd":"%s","Expiretime":"%s"}'%\
			(self.__appId,self.__accountToken,phoneNumber,openId,create_time)
		#print("body:",body)
	...
	def __analyzing(self,result,phone):
		if len(result) == 2:
			...
			resl['createDate'] = (data['templateSMS'])['createDate']
