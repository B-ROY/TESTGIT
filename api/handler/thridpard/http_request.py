...
class RequestApi(object):
    ...
    @classmethod
    def post_body_request_http(cls, path, body, headers={}, host=''):
        method = "POST"
    ...
    @classmethod
    def post_body_request_async(cls, path, body, headers={}, host=""):
        method = "POST"

        _headers = {'Accept-Language': 'zh-cn', 'User-Agent': 'Python/Automate', "Accept-Charset": "utf-8"}
        _headers.update(headers)
        asynchttpclient = AsyncHTTPClient()
        asynchttpclient.fetch(path, method=method, body=body,callback=cls.handle_post_async)
        tornado.ioloop.IOLoop.instance().start()



    @classmethod
    def handle_post_async(cls,response):
        print 1
        print response.body
