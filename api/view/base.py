...
from live_video.base.core.library import cache
from tornado.web import HTTPError
from app.customer.models import User
from api.apiexceptions.apiexception import ApiException
import logging
...
class BaseHandler(RequestHandler):
    ...
    def _deco_method(self, fn):
        def _func(*args, **kwargs):
            ...
            try:
                ...
                logging.error(e)
                return self.send_error(status_code=400, desc=e.desc, code=e.code)
        return _func
    ...
    def user_ip(self):
        if not hasattr(self, '_real_ip'):
            ...
            #TODO 这个是临时解决方案，因为服务器出现了两个IP，导致微信无法支付
            if "," in self._real_ip:
                self._real_ip = self._real_ip.split(",")[0]
...
class CachedHandler(BaseHandler):
    ...
    enable_cache = settings.enable_memcached
    ...
    def _deco_get(self, fn):
        def _func(*args):
            ...
            if not self.enable_cache:
                ...
                value = None if self.is_update_cache else cache.page_cache.get(ckey)
                if value is None:
                    fn(*args)
                    if self.get_status() in (200, 304) and self._result_buffer:
                        cache.page_cache.set(ckey, self._result_buffer,
    ...
    def _deco_get(self, fn):
        def _func(*args):
            ...
            if not self.enable_cache:
                ...
                expire_s = None
                cached_result = cache.get_plus_json(
