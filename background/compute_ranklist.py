...
def compute_7_rank_list_first():
    CharmRank.objects.filter(type=1).delete()
    WealthRank.objects.filter(type=1).delete()
    ...
    for charm_record in charm_record_list:
        if charm_record.user.id in charm_rank_list:
            ...
            charm_rank = CharmRank(user=charm_record.user, charm=charm_record.ticket/10, type=1)
    ...
    for wealth_record in wealth_record_list:
        if wealth_record.user.id in wealth_rank_list:
            ...
            wealth_rank = WealthRank(user=wealth_record.user, wealth=wealth_record.diamon/10, type=1)
...
def compute_7_rank_list_delta():
    ...
    old_charm_ranks = CharmRank.objects.filter(type=1)
    old_wealth_ranks = WealthRank.objects.filter(type=1)
...
def compute_1_rank_list_first():
    ...
    CharmRank.objects.filter(type=2).delete()
    WealthRank.objects.filter(type=2).delete()
    ...
    for charm_record in charm_record_list:
        if charm_record.user.id in charm_rank_list:
            ...
            charm_rank = CharmRank(user=charm_record.user, charm=charm_record.ticket/10, type=2)
            charm_rank_list[charm_record.user.id] = charm_rank
    ...
    for wealth_record in wealth_record_list:
        if wealth_record.user.id in wealth_rank_list:
            ...
            wealth_rank = WealthRank(user=wealth_record.user, wealth=wealth_record.diamon/10, type=2)
            wealth_rank_list[wealth_record.user.id] = wealth_rank
...
def compute_1_rank_list_delta():
    ...
    old_charm_ranks = CharmRank.objects.filter(type=2)
    old_wealth_ranks = WealthRank.objects.filter(type=2)
    for old_charm in old_charm_ranks:
        old_charm_user_ids[old_charm.user.id] = old_charm.rank
