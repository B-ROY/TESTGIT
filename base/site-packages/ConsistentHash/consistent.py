...
if __name__ == "__main__":
    ...
    now = time.time()
    rhclient = RedisHashClient(hosts)
    print "initial use:", time.time() - now

    now = time.time()
    a=  rhclient.hset("test", "attr", " 1")
    print "#######",a
    print "hset use:", time.time() - now
    now = time.time()
    data = rhclient.hget("test", "attr")
    print "hget use:", time.time() - now
    print data
...
#        print rhclient.hget("test%s" % i, "attr"), "deal %s" % i, "\r"
#
#    print "hget use:", time.time() - now, (time.time() - now) * 1.0 / test_count
